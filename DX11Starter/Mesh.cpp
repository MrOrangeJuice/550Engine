...
// Constructor
Mesh::Mesh(Vertex vertices[], int vertNum, unsigned int indices[], int indNum, ID3D11Device* bufferCreation)
{
	...
	CalculateTangents(vertices, vertNum, indices, indNum);
	...
	vbd.ByteWidth = sizeof(Vertex) * vertNum;       // 3 = number of vertices in the buffer
	...
	bufferCreation->CreateBuffer(&vbd, &initialVertexData, &vertexBuffer);
	...
	ibd.ByteWidth = sizeof(unsigned int) * indNum;	// 3 = number of indices in the buffer
	...
	// Actually create the buffer with the initial data
	// - Once we do this, we'll NEVER CHANGE THE BUFFER AGAIN
	bufferCreation->CreateBuffer(&ibd, &initialIndexData, &indexBuffer);

	this->indNum = indNum;
}

Mesh::Mesh(const char* fileName, ID3D11Device* bufferCreation)
{
